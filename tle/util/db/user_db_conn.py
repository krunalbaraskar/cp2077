...
class UserDbConn:
    ...
    def check_duel_complete(self, userid):
        query = f"""
            SELECT
                ...
                contest_id, p_index, type, nohandicap FROM duel
            WHERE (challenger = ? OR challengee = ?) AND status == {Duel.ONGOING}
        """
        return self.conn.execute(query, (userid, userid)).fetchone()
    ...
    def create_duel(self, challenger, challengee, issue_time, prob, dtype, nohandicap=False):
        query = f"""
            INSERT INTO duel (
                ...
                p_index, status, type, nohandicap
            ) VALUES (?, ?, ?, ?, ?, ?, {Duel.PENDING}, ?, ?)
        """
    ...
    def create_multiplayer_duel(self, creator_id, guild_id, issue_time, num_problems, rating, dtype, nohandicap=False):
        ...
        query = f"""
            INSERT INTO multiplayer_duel (
                creator_id, guild_id, issue_time, num_problems, rating, status, type, nohandicap
            ) VALUES (?, ?, ?, ?, ?, {Duel.PENDING}, ?, ?)
        """
    ...
    def add_multiplayer_participant(self, duel_id, user_id, status=ParticipantStatus.INVITED):
        ...
        query = """
            INSERT OR IGNORE INTO multiplayer_duel_participant (
                duel_id, user_id, status, problems_solved, total_time
            ) VALUES (?, ?, ?, 0, 0)
        """
    ...
    def add_multiplayer_problem(self, duel_id, problem, problem_order):
        ...
        query = """
            INSERT INTO multiplayer_duel_problem (
                duel_id, problem_name, contest_id, p_index, problem_order
            ) VALUES (?, ?, ?, ?, ?)
        """
        rc = self.conn.execute(
    ...
    def check_multiplayer_duel_participant(self, user_id):
        ...
        query = f"""
            SELECT p.duel_id, d.status
            FROM multiplayer_duel_participant p
            JOIN multiplayer_duel d ON p.duel_id = d.id
            WHERE p.user_id = ? 
            AND d.status IN ({Duel.PENDING}, {Duel.ONGOING})
    ...
    def get_multiplayer_duel(self, duel_id):
        ...
        query = """
            SELECT id, creator_id, guild_id, issue_time, start_time, 
                   finish_time, status, type, num_problems, rating, nohandicap
            FROM multiplayer_duel
            WHERE id = ?
        """
    ...
    def get_multiplayer_duel_by_user(self, user_id):
        ...
        query = f"""
            SELECT d.id, d.creator_id, d.guild_id, d.issue_time, d.start_time,
                   d.finish_time, d.status, d.type, d.num_problems, d.rating, d.nohandicap
            FROM multiplayer_duel d
            JOIN multiplayer_duel_participant p ON d.id = p.duel_id
            WHERE p.user_id = ? 
            AND d.status IN ({Duel.PENDING}, {Duel.ONGOING})
            AND p.status != {ParticipantStatus.DECLINED}
        """
    ...
    def get_multiplayer_participants(self, duel_id):
        ...
        query = """
            SELECT user_id, status, problems_solved, total_time, placement, rating_delta
            FROM multiplayer_duel_participant
            WHERE duel_id = ?
            ORDER BY user_id
        """
    ...
    def check_all_accepted(self, duel_id):
        ...
        query = f"""
            SELECT COUNT(*) 
            FROM multiplayer_duel_participant 
            WHERE duel_id = ? AND status = {ParticipantStatus.INVITED}
        """
    ...
    def start_multiplayer_duel(self, duel_id, start_time):
        ...
        query = f"""
            UPDATE multiplayer_duel 
            SET start_time = ?, status = {Duel.ONGOING}
            WHERE id = ? AND status = {Duel.PENDING}
        """
        rc = self.conn.execute(query, (start_time, duel_id)).rowcount
        if rc != 1:
            self.conn.rollback()
            return 0
        self.conn.commit()
    ...
    def complete_multiplayer_duel(self, duel_id, finish_time, placements_with_deltas, dtype):
        ...
        query = f"""
            UPDATE multiplayer_duel 
            SET status = {Duel.COMPLETE}, finish_time = ?
            WHERE id = ? AND status = {Duel.ONGOING}
        """
        rc = self.conn.execute(query, (finish_time, duel_id)).rowcount
        if rc != 1:
            self.conn.rollback()
            return 0
    ...
    def cancel_multiplayer_duel(self, duel_id, status):
        ...
        query = f"""
            ...
            WHERE id = ? AND status = {Duel.PENDING}
        """
        rc = self.conn.execute(query, (status, duel_id)).rowcount
        if rc != 1:
            self.conn.rollback()
            return 0
        self.conn.commit()
    ...
    def get_multiplayer_duel_history(self, user_id):
        ...
        query = f"""
            SELECT d.id, d.start_time, d.finish_time, d.num_problems, 
                   p.placement, p.problems_solved, p.total_time, p.rating_delta
            FROM multiplayer_duel d
            JOIN multiplayer_duel_participant p ON d.id = p.duel_id
            WHERE p.user_id = ? AND d.status = {Duel.COMPLETE}
