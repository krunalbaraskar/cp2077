...
class SupabaseDbConn:
    ...
    def fetch_cf_user(self, handle):
        result = (
            self.client.table('cf_user_cache')
            .select('*')
            .ilike('handle', handle)
            .execute()
        )
    ...
    def register_duelist(self, user_id):
        result = (
            self.client.table('duelist')
            .upsert({'user_id': user_id, 'rating': 1500}, on_conflict='user_id')
            .execute()
        )
        return len(result.data)
    ...
    def check_duel_challenge(self, userid):
        result = (
            self.client.table('duel')
            .select('id')
            .or_(f'challenger.eq.{userid},challengee.eq.{userid}')
            .in_('status', [Duel.ONGOING, Duel.PENDING])
            .execute()
        )
    ...
    def check_duel_accept(self, challengee):
        result = (
            self.client.table('duel')
            .select('id, challenger, problem_name')
            .eq('challengee', challengee)
            .eq('status', Duel.PENDING)
            .execute()
        )
        if result.data:
    ...
    def check_duel_withdraw(self, challenger):
        result = (
            self.client.table('duel')
            .select('id, challengee')
            .eq('challenger', challenger)
            .eq('status', Duel.PENDING)
            .execute()
        )
        if result.data:
    ...
    def check_duel_draw(self, userid):
        result = (
            self.client.table('duel')
            .select('id, challenger, challengee, start_time')
            .or_(f'challenger.eq.{userid},challengee.eq.{userid}')
            .eq('status', Duel.ONGOING)
            .execute()
        )
        if result.data:
            return _make_row(
    ...
    def start_duel(self, duelid, start_time):
        result = (
            self.client.table('duel')
            .update({'start_time': start_time, 'status': Duel.ONGOING})
            .eq('id', duelid)
            .eq('status', Duel.PENDING)
            .execute()
        )
    ...
    def complete_duel(
        ...
        result = (
            self.client.table('duel').update(update_data).eq('id', duelid).execute()
        )
        return len(result.data)
    ...
    def get_duel_wins(self, userid):
        ...
        as_challengee = (
            self.client.table('duel')
            .select('id', count='exact')
            .eq('challengee', userid)
            .eq('status', Duel.COMPLETE)
            .eq('winner', Winner.CHALLENGEE)
            .execute()
        )
    ...
    def get_ongoing_duels(self):
        result = (
            self.client.table('duel')
            .select('*')
            .in_('status', [Duel.PENDING, Duel.ONGOING])
            .execute()
        )
        return [_make_row(r) for r in result.data]
    ...
    def get_duel_problem_names(self, userid):
        ...
        result = (
            ...
            .select('problem_name')
            .or_(f'challenger.eq.{userid},challengee.eq.{userid}')
            .in_('status', [Duel.COMPLETE, Duel.INVALID])
            .execute()
        )
    ...
    def get_complete_official_duels(self):
        ...
        result = (
            ...
            .eq('status', Duel.COMPLETE)
            .eq('type', DuelType.OFFICIAL)
            .order('finish_time')
            .execute()
        )
        return [
            _make_row(r, ['challenger', 'challengee', 'winner', 'finish_time'])
    ...
    def check_multiplayer_duel_participant(self, user_id):
        ...
        result = (
            self.client.table('multiplayer_duel_participant')
            .select('duel_id')
            .eq('user_id', user_id)
            .neq('status', ParticipantStatus.DECLINED)
            .execute()
        )
        ...
        duels = (
            self.client.table('multiplayer_duel')
            .select('id, status')
            .in_('id', duel_ids)
            .in_('status', [Duel.PENDING, Duel.ONGOING])
            .limit(1)
    ...
    def update_participant_progress(
        ...
        result = (
            self.client.table('multiplayer_duel_participant')
            .update({'problems_solved': problems_solved, 'total_time': total_time})
            .eq('duel_id', duel_id)
            .eq('user_id', user_id)
            .execute()
        )
    ...
    def new_challenge(self, user_id, issue_time, prob, delta):
        ...
        self.client.table('user_challenge').upsert(
            {'user_id': str(user_id), 'score': 0, 'num_completed': 0, 'num_skipped': 0},
            on_conflict='user_id',
        ).execute()

        # Update only if no active challenge
        existing = (
            self.client.table('user_challenge')
        ...
        self.client.table('user_challenge').update(
            {'active_challenge_id': challenge_id, 'issue_time': issue_time}
    ...
    def check_challenge(self, user_id):
        uc = (
            self.client.table('user_challenge')
            .select('active_challenge_id, issue_time')
            .eq('user_id', str(user_id))
            .execute()
        )
        ...
        challenge = (
            self.client.table('challenge')
            .select('problem_name, contest_id, p_index, rating_delta')
            .eq('id', c_id)
            .execute()
        )
    ...
    def complete_challenge(self, user_id, challenge_id, finish_time, delta):
        ...
        self.client.table('challenge').update(
            {'finish_time': finish_time, 'status': Gitgud.GOTGUD}
        ).eq('id', challenge_id).eq('status', Gitgud.GITGUD).execute()

        # Update user stats
        uc = (
            self.client.table('user_challenge')
            .select('score, num_completed')
            .eq('user_id', str(user_id))
            .execute()
        )
    ...
    def skip_challenge(self, user_id, challenge_id, status):
        self.client.table('user_challenge').update(
            {'active_challenge_id': None, 'issue_time': None}
        ).eq('user_id', str(user_id)).execute()

        self.client.table('challenge').update({'status': status}).eq(
            'id', challenge_id
        ).eq('status', Gitgud.GITGUD).execute()
    ...
    def gitlog(self, user_id):
        result = (
            ...
            .eq('user_id', str(user_id))
            .neq('status', Gitgud.FORCED_NOGUD)
    ...
    def get_reminder_settings(self, guild_id):
        result = (
            self.client.table('reminder')
            .select('channel_id, role_id, before')
            .eq('guild_id', str(guild_id))
            .execute()
        )
        if result.data:
            return _make_row(result.data[0], ['channel_id', 'role_id', 'before'])
        return None
    ...
    def has_auto_role_update_enabled(self, guild_id):
        result = (
            self.client.table('auto_role_update')
            .select('guild_id')
            .eq('guild_id', str(guild_id))
            .execute()
        )
    ...
    def get_starboard_entry(self, guild_id, emoji):
        cfg = (
            self.client.table('starboard_config_v1')
            .select('channel_id')
            .eq('guild_id', str(guild_id))
            .eq('emoji', emoji)
            .execute()
        ...
        emo = (
            self.client.table('starboard_emoji_v1')
        ...
        return (
            int(cfg.data[0]['channel_id']),
            int(emo.data[0]['threshold']),
    ...
    def set_starboard_channel(self, guild_id, emoji, channel_id):
        self.client.table('starboard_config_v1').upsert(
            {'guild_id': str(guild_id), 'emoji': emoji, 'channel_id': str(channel_id)}
        ).execute()
        return 1
    ...
    def check_exists_starboard_message(self, original_msg_id, emoji):
        result = (
            self.client.table('starboard_message_v1')
            .select('original_msg_id')
            .eq('original_msg_id', str(original_msg_id))
            .eq('emoji', emoji)
            .execute()
        )
        return bool(result.data)
    ...
    def remove_starboard_message(
        ...
        if original_msg_id is not None and emoji is not None:
            ...
            result = (
                self.client.table('starboard_message_v1')
                .delete()
                .eq('starboard_msg_id', str(starboard_msg_id))
                .execute()
            )
        else:
            return 0
    ...
    def add_multiplayer_participant(
        ...
        self.client.table('multiplayer_duel_participant').upsert(
            {'duel_id': duel_id, 'user_id': user_id, 'status': status}
        ).execute()
    ...
    def get_multiplayer_problems(self, duel_id):
        result = (
            self.client.table('multiplayer_duel_problem')
            .select('problem_name, contest_id, p_index, problem_order')
            .eq('duel_id', duel_id)
            .order('problem_order')
            .execute()
        )
    ...
    def complete_multiplayer_duel(
        ...
        self.client.table('multiplayer_duel').update(
            {'finish_time': finish_time, 'status': Duel.COMPLETE}
        ...
        for user_id, placement, rating_delta in placements_with_deltas:
            self.client.table('multiplayer_duel_participant').update(
                {'placement': placement, 'rating_delta': rating_delta}
            ).eq('duel_id', duel_id).eq('user_id', user_id).execute()
    ...
    def get_multiplayer_duel_history(self, user_id):
        result = (
            self.client.table('multiplayer_duel_participant')
            .select('duel_id, problems_solved, total_time, placement, rating_delta')
            .eq('user_id', user_id)
            .execute()
        )
    ...
    def finish_rated_vc(self, vc_id, finish_time):
        self.client.table('rated_vcs').update(
            {'finish_time': finish_time, 'status': RatedVC.FINISHED}
        ).eq('id', vc_id).execute()
    ...
    def add_rated_vc_user(self, vc_id, user_id, rating):
        self.client.table('rated_vc_users').upsert(
            {'vc_id': vc_id, 'user_id': str(user_id), 'rating': rating}
        ).execute()

    def get_rated_vc_users(self, vc_id):
        result = (
            self.client.table('rated_vc_users')
    ...
    def get_rated_vc_channel(self, guild_id):
        result = (
            self.client.table('rated_vc_settings')
            .select('channel_id')
            .eq('guild_id', str(guild_id))
            .execute()
        )
